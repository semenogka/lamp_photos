...
class Worker(QThread):
    ...
    def __init__(self, img, cats, widgth, height, abajurcolors, metalcolors, _cache, abajur_colors_dict, metal_colors_dict):
        super().__init__()
                ...
                for embs_img, labels_img in zip(embs, labels):
                    for tf1, emb_user in zip(labels_img, embs_img):
                        ...
                        if text_sim > 0.9:
                            ...
                            image_sim = torch.cosine_similarity(emb_user, emb_array[i], dim=-1)
                            minres.append({
                                "sim": image_sim.item() * 100,
                                "link": item['link'],
                                "name": item['name']
                            })
                if minres:
