...
def get_files():
    ...
    for file in buttons_files:
        full_local_path = buttons_files[file]
        filename = os.path.basename(full_local_path)
        raw_url = f"https://raw.githubusercontent.com/semenogka/lamp_photos/refs/heads/main/alldata/ndata/{filename}"
        local_path = resource_path(os.path.join(FOLDER, filename))
        ...
        if remote_hash != local_hash:
            with open(local_path, "wb") as f:
                f.write(res.content)
                print(f"Update: {file}")
                update_cache(file, resource_path("cache.pt"))

def load_cache():
    global _cache
    if os.path.exists(resource_path("cache.pt")):
        _cache = torch.load(resource_path("cache.pt"))
        print("loaded")
    else:
        print("where cache.pt")        


def update_cache(category, path="cache.pt"):
    if os.path.exists(resource_path(path)):
        torch_cache = torch.load(path)
    else:
        torch_cache = {}
    ...
    if category in _cache:
        ...
        torch_cache[category] = {
            "data": data,
            "text_embs": text_embs,
            "emb_array": emb_array
        }
        print("ready ", category)   
    torch.save(torch_cache, "cache.pt")
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        print("скачивание данных о светильниках и их кеширование.")
        print("кеш позволяет находить скожие светильники быстрее и проще для компьютера.")
        print("обновление кеша может сильно нагрузить компьютер")
        load_cache()
        get_files()
